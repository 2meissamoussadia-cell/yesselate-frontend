// app/api/alerts/events/[id]/close/route.ts
// Phase P15: Moteur d'alertes - Fermer un événement
...
const log = withReq('alerts-events-close');

/**
 * POST /api/alerts/events/[id]/close
 * 
 * Fermer un événement d'alerte
...
export async function POST(
  ...
  const rl = await rateLimitRedis(`alerts:events:close:${ip}`, 60, 1);
  if (!rl.allowed) {
  ...
  try {
    ...
    // Guard: alerts:view minimum, alerts:admin peut fermer toutes les alertes
    if (!can(ctx, { perm: 'alerts:view' })) {
      const seconds = (performance.now() - t0) / 1000;
      logReq.info({ seconds, error: 'Forbidden: alerts:view required' }, 'alerts event close forbidden');
    ...
    try {
      ...
      // ABAC: Vérifier que l'utilisateur peut fermer cet événement
      // Si admin, peut tout fermer. Sinon, vérifier les scopes bureau/chantier
      if (!isAdmin && ctx.scopes && ctx.scopes.length > 0) {
        ...
        if (!canAccess) {
          const seconds = (performance.now() - t0) / 1000;
          logReq.info({ seconds, error: 'Forbidden: ABAC scope mismatch' }, 'alerts event close forbidden');
      ...
      if (result.rows.length === 0) {
        return NextResponse.json(
          { error: 'Event not found or already closed', ok: false },
          { status: 404 }
        );
      }
      
      const seconds = (performance.now() - t0) / 1000;
      logReq.info({ eventId, seconds }, 'alerts event closed');
    ...
    const seconds = (performance.now() - t0) / 1000;
    logReq.error({ err: error, seconds }, 'alerts event close error');
    return NextResponse.json(
      { error: 'Failed to close event', ok: false },
      { status: 500 }
    );
  }
}
