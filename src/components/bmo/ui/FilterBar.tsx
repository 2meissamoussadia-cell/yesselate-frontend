...
export interface FilterBarProps {
  ...
  /** Nombre de filtres avancés actifs (badge sur le bouton Filtre) */
...
export const FilterBar = React.memo(function FilterBar({
  ...
  const quickFilterCount = Array.isArray(activeFilters)
    ? activeFilters.length
    : Object.values(activeFilters).filter(Boolean).length;
  const activeFilterCount = advancedFilterCountProp ?? quickFilterCount;
  ...
  return (
    <div className={cn('bg-white dark:bg-slate-950/40 border-b border-slate-200 dark:border-slate-800', className)}>
      {/* Une seule ligne : onglets + Filtre + Date création dans la même zone (bleu) */}
      <div className="flex flex-wrap items-center gap-2 px-4 py-2">
        ...
        {(onAdvancedFilterClick != null || sortLabel != null) && viewTabs.length > 0 && (
          <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 shrink-0" aria-hidden />
        )}

        <div className="flex items-center gap-2 shrink-0">
          {onAdvancedFilterClick != null && (
            <Button
              variant="outline"
              size="sm"
