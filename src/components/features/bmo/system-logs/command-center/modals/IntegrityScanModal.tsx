...
export function IntegrityScanModal({ open, onClose }: IntegrityScanModalProps) {
  if (!open) return null;
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800/50 bg-slate-800/30">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-100">Scan d’intégrité</h2>
          </div>
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-slate-400">Vérification de l’intégrité des logs et des index. À brancher sur un job backend.</p>
          <div className="flex justify-end gap-2">
            <Button variant="outline" size="sm" onClick={onClose}>Fermer</Button>
            <Button size="sm" onClick={onClose}>Lancer le scan</Button>
          </div>
        </div>
      </div>
    </div>
