...
export function EmployesCommandPalette({ open, onClose, onOpenStats, onRefresh }: Props) {
  const { openTab, clearFilter } = useEmployesWorkspaceStore(); const [query, setQuery] = useState(''); const [selectedIndex, setSelectedIndex] = useState(0); const inputRef = useRef<HTMLInputElement>(null);
  const commands: Command[] = useMemo(() => [
    { id: 'nav-all', label: 'Tous les employés', icon: Users, category: 'navigation', shortcut: '⌘1', action: () => { openTab({ type: 'inbox', title: 'Tous', icon: '👥', data: { queue: 'all' } }); onClose(); } },
    { id: 'nav-actifs', label: 'Actifs', icon: CheckCircle, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:actif', title: 'Actifs', icon: '✅', data: { queue: 'actif' } }); onClose(); } },
    { id: 'nav-conges', label: 'En congés', icon: Calendar, category: 'navigation', action: () => { openTab({ type: 'absences', title: 'Congés', icon: '📅', data: {} }); onClose(); } },
    { id: 'nav-mission', label: 'En mission', icon: Plane, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:mission', title: 'Mission', icon: '✈️', data: { queue: 'mission' } }); onClose(); } },
    { id: 'nav-spof', label: 'SPOF (Single Point of Failure)', description: 'Compétences critiques uniques', icon: Shield, category: 'navigation', shortcut: '⌘2', action: () => { openTab({ type: 'spof', title: 'SPOF', icon: '🛡️', data: {} }); onClose(); } },
    { id: 'nav-risk', label: 'À risque', icon: AlertTriangle, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:risk', title: 'À risque', icon: '⚠️', data: { queue: 'risk' } }); onClose(); } },
    { id: 'nav-eval', label: 'Évaluations', icon: Star, category: 'navigation', action: () => { openTab({ type: 'evaluations', title: 'Évaluations', icon: '⭐', data: {} }); onClose(); } },
  ...
  return createPortal(<div className="fixed inset-0 z-[100] flex items-start justify-center pt-[12vh] bg-black/60 backdrop-blur-sm" onClick={onClose}><div className="w-full max-w-2xl mx-4 rounded-2xl border border-slate-200/70 bg-white/95 backdrop-blur-xl shadow-2xl dark:border-slate-800 dark:bg-[#1f1f1f]/95 overflow-hidden" onClick={e => e.stopPropagation()}><div className="flex items-center gap-3 px-4 py-4 border-b border-slate-200/70 dark:border-slate-800"><Search className="w-5 h-5 text-teal-500 flex-none" /><input ref={inputRef} type="text" placeholder="Rechercher..." value={query} onChange={e => setQuery(e.target.value)} onKeyDown={handleKeyDown} className="flex-1 bg-transparent outline-none text-lg" /><kbd className="px-2 py-1 rounded bg-slate-100 dark:bg-slate-800 text-xs font-mono">ESC</kbd></div><div className="max-h-[60vh] overflow-y-auto py-2">{groupedCommands.map(({ category, items }) => (<div key={category} className="mb-1"><div className="px-4 py-2 text-xs font-semibold text-slate-400 uppercase">{categoryLabels[category]}</div>{items.map(cmd => { const globalIdx = filteredCommands.indexOf(cmd); const isSelected = globalIdx === selectedIndex; const Icon = cmd.icon; return (<button key={cmd.id} onClick={cmd.action} onMouseEnter={() => setSelectedIndex(globalIdx)} className={cn("w-full flex items-center gap-3 px-4 py-3 text-left", isSelected ? "bg-teal-500/10 text-teal-600 dark:text-teal-400" : "hover:bg-slate-100 dark:hover:bg-slate-800/60")}><div className={cn("p-2 rounded-lg", isSelected ? "bg-teal-500/20" : "bg-slate-100 dark:bg-slate-800")}><Icon className="w-4 h-4" /></div><div className="flex-1 min-w-0"><div className="font-medium">{cmd.label}</div>{cmd.description && <div className="text-xs text-slate-400">{cmd.description}</div>}</div>{cmd.shortcut && <kbd className={cn("px-2 py-1 rounded text-xs font-mono", isSelected ? "bg-teal-500/20" : "bg-slate-100 dark:bg-slate-800")}>{cmd.shortcut}</kbd>}<ArrowRight className={cn("w-4 h-4", isSelected ? "opacity-100" : "opacity-0")} /></button>); })}</div>))}</div></div></div>, document.body);
}
