...
export const DashboardBreadcrumbs = memo(function DashboardBreadcrumbs({ className }: { className?: string }) {
  ...
  return (
    <nav className={cn('flex items-center gap-1.5 text-xs', className)} aria-label="Fil d'Ariane">
      <button
        type="button"
      ...
      <button
        type="button"
        onClick={() => go({ main })}
        className="rounded-md px-1.5 py-0.5 text-slate-200 transition-colors hover:bg-slate-900/40 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
      >
        {mainLabel}
      </button>
      ...
      {leafLabel ? (
        <>
          <ChevronRight className="h-3 w-3 text-slate-600" />
          <span className="rounded-md px-1.5 py-0.5 font-medium text-slate-100">{leafLabel}</span>
        </>
      ) : null}
    </nav>
  );
