...
export function CollapsibleSection({
  ...
  return (
    <div
      id={id}
      className={cn(
        'rounded-2xl border border-slate-800/80 bg-slate-950/80 overflow-hidden transition-all duration-300',
    ...
    >
      <button
        ...
        className={cn(
          ...
          'hover:bg-slate-900/40 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/50 focus-visible:ring-inset',
          'text-left'
        )}
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {itemCount != null && (
            <span className="text-[11px] text-slate-400 rounded-full bg-slate-800/60 px-2 py-0.5">
      ...
      >
        <div className="overflow-hidden">
          <div className="border-t border-slate-800/60 px-4 pb-4 pt-3">{children}</div>
        </div>
      </div>
    </div>
  );
