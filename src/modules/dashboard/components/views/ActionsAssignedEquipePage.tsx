/**
 * Page Actions Assignées à l'équipe
 * Actions assignées à l'équipe
 */
...
export const ActionsAssignedEquipePage = memo(function ActionsAssignedEquipePage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `actions-assignees-equipe-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `actions-assignees-equipe-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || "Impossible de charger les actions de l'équipe"}
          icon={FileSignature}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions assignées à l'équipe" description="Actions assignées à l'équipe">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune action équipe"
              description="Il n'y a actuellement aucune action assignée à l'équipe."
              icon={FileText}
              variant="info"
            />
          ) : (
