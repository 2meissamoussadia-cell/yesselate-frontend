...
export const ActionsAssignedMoiPage = memo(function ActionsAssignedMoiPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `actions-assignees-moi-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `actions-assignees-moi-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les actions qui me sont assignées'}
          icon={FileSignature}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions assignées à moi" description="Actions qui me sont assignées">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              ...
              description="Il n'y a actuellement aucune action qui vous est assignée."
              icon={FileText}
              variant="info"
            />
          ) : (
