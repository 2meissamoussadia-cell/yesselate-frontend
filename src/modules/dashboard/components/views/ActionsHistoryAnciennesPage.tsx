/**
 * Page Actions Historique anciennes
 * Actions anciennes
 */
...
export const ActionsHistoryAnciennesPage = memo(function ActionsHistoryAnciennesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `actions-anciennes-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `actions-anciennes-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les actions anciennes'}
          icon={FileSignature}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions anciennes" description="Historique des actions anciennes">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune action ancienne"
              description="Il n'y a actuellement aucune action ancienne."
              icon={FileText}
              variant="info"
            />
          ) : (
