/**
 * Page Actions Historique archivées
 * Actions archivées
 */
...
export const ActionsHistoryArchiveesPage = memo(function ActionsHistoryArchiveesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `actions-archivees-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `actions-archivees-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les actions archivées'}
          icon={FileSignature}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions archivées" description="Historique des actions archivées">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune action archivée"
              description="Il n'y a actuellement aucune action archivée."
              icon={FileText}
              variant="info"
            />
          ) : (
