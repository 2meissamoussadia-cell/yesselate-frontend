/**
 * Page Actions Inbox Urgentes
 * Vue des actions urgentes dans la boîte de réception
 */

'use client';

import React, { memo, useMemo, useState, useCallback } from 'react';
import { AlertCircle, Clock, Zap, FileText, DollarSign, Briefcase, Gavel } from 'lucide-react';
import { 
  DashboardPageLayout, 
  DashboardSection, 
  DashboardPanel,
...
import type { ActionsInboxUrgentesData } from '../../types/dashboardDataTypes';
import { cn } from '@/lib/utils';

export const ActionsInboxUrgentesPage = memo(function ActionsInboxUrgentesPage() {
  const [searchQuery, setSearchQuery] = useState('');
  
  // ✅ Charger les données depuis l'API
  const { data, isLoading, error } = useDashboardData<ActionsInboxUrgentesData>();
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rows, headers, `actions-urgentes-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredActions]);
  
  // ✅ Export JSON
  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredActions, `actions-urgentes-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredActions]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les actions urgentes'}
          icon={AlertCircle}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions Urgentes" description="Actions urgentes nécessitant une attention immédiate">
        ...
        <DashboardPanel>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-slate-200">Liste des actions urgentes</h3>
          ...
          {filteredActions.length === 0 ? (
            <EmptyState
              title={searchQuery ? "Aucune action trouvée" : "Aucune action urgente"}
              description={
                searchQuery
                  ? `Aucune action urgente ne correspond à "${searchQuery}"`
                  : "Il n'y a actuellement aucune action urgente dans votre boîte de réception."
              }
              icon={AlertCircle}
              variant="warning"
            />
