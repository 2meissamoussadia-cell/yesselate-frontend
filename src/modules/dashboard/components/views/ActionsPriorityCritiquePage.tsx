/**
 * Page Actions Priorité Critique
 * Actions à priorité critique
 */
...
export const ActionsPriorityCritiquePage = memo(function ActionsPriorityCritiquePage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `actions-priority-critique-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `actions-priority-critique-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);

  const kpis: KPICardData[] = useMemo(
    () => [{ id: 'total', label: 'Total', value: stats.total, color: 'red', trend: '+0%' }],
    [stats.total]
  );
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les actions priorité critique'}
          icon={FileSignature}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions Priorité Critique" description="Actions à priorité critique">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune action priorité critique"
              description="Il n'y a actuellement aucune action à priorité critique."
              icon={FileText}
              variant="info"
            />
          ) : (
