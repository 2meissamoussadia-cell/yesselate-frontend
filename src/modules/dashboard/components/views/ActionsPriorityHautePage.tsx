/**
 * Page Actions Priorité Haute
 * Actions à priorité haute
 */
...
export const ActionsPriorityHautePage = memo(function ActionsPriorityHautePage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `actions-priority-haute-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `actions-priority-haute-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);

  const kpis: KPICardData[] = useMemo(
    () => [{ id: 'total', label: 'Total', value: stats.total, color: 'orange', trend: '+0%' }],
    [stats.total]
  );
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les actions priorité haute'}
          icon={FileSignature}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions Priorité Haute" description="Actions à priorité haute">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune action priorité haute"
              description="Il n'y a actuellement aucune action à priorité haute."
              icon={FileText}
              variant="info"
            />
          ) : (
