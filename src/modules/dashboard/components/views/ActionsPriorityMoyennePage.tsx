/**
 * Page Actions Priorité Moyenne
 * Actions à priorité moyenne
 */
...
export const ActionsPriorityMoyennePage = memo(function ActionsPriorityMoyennePage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `actions-priority-moyenne-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `actions-priority-moyenne-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les actions priorité moyenne'}
          icon={FileSignature}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions Priorité Moyenne" description="Actions à priorité moyenne">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune action priorité moyenne"
              description="Il n'y a actuellement aucune action à priorité moyenne."
              icon={FileText}
              variant="info"
            />
          ) : (
