/**
 * Page Admin — Logs système
 * Logs techniques et erreurs du tableau de bord.
 */
...
interface AdminLogsSystemeData {
  items?: { id: string; level: string; message: string; at: string }[];
  total?: number;
}

export const AdminLogsSystemePage = memo(function AdminLogsSystemePage(props: { data?: AdminLogsSystemeData | null }) {
  const [searchQuery, setSearchQuery] = React.useState('');
  const raw = props.data as AdminLogsSystemeData | undefined;
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Logs système" description="Logs techniques et erreurs du tableau de bord.">
        ...
        <DashboardPanel>
          ...
          {filtered.length === 0 ? (
            <EmptyState
              title="Aucun log"
              description={items.length === 0 ? "Aucun log système." : "Aucun résultat pour cette recherche."}
              icon={FileText}
              variant="info"
            />
          ) : (
                  ...
                  {filtered.map((l) => (
                    <tr key={l.id} className="border-t border-slate-700/50 hover:bg-slate-800/30">
                      ...
                      <td className="px-4 py-3">
                        <span className={l.level === 'error' ? 'text-red-400' : l.level === 'warn' ? 'text-amber-400' : 'text-slate-300'}>{l.level}</span>
                      </td>
