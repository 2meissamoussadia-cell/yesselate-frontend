/**
 * Page Rapports d'Audit
 * Rapports d'audit des décisions
 */
...
export const DecisionsAuditRapportsPage = memo(function DecisionsAuditRapportsPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `audit-rapports-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `audit-rapports-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les rapports d\'audit'}
          icon={FileBarChart}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Rapports d'Audit" description="Rapports d'audit des décisions">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucun rapport d'audit"
              description="Il n'y a actuellement aucun rapport d'audit."
              icon={FileText}
              variant="info"
            />
          ) : (
