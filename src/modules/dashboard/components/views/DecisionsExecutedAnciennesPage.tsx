/**
 * Page Décisions Anciennes
 * Historique des décisions exécutées
 */
...
export const DecisionsExecutedAnciennesPage = memo(function DecisionsExecutedAnciennesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-anciennes-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-anciennes-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les décisions anciennes'}
          icon={History}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Décisions Anciennes" description="Historique des décisions exécutées">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune décision ancienne"
              description="Il n'y a actuellement aucune décision dans l'historique."
              icon={FileText}
              variant="info"
            />
          ) : (
