/**
 * Page Décisions Par Type
 * Décisions exécutées regroupées par type
 */
...
export const DecisionsExecutedParTypePage = memo(function DecisionsExecutedParTypePage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-par-type-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-par-type-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les décisions par type'}
          icon={Layers}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Décisions Par Type" description="Décisions exécutées regroupées par type">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune donnée"
              description="Il n'y a actuellement aucune décision par type."
              icon={FileText}
              variant="info"
            />
          ) : (
