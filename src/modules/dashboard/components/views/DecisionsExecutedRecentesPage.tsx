/**
 * Page Décisions Récentes
 * Décisions exécutées récemment
 */
...
export const DecisionsExecutedRecentesPage = memo(function DecisionsExecutedRecentesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-recentes-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-recentes-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les décisions récentes'}
          icon={Clock}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Décisions Récentes" description="Décisions exécutées récemment">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              ...
              description="Il n'y a actuellement aucune décision exécutée récemment."
              icon={FileText}
              variant="info"
            />
          ) : (
