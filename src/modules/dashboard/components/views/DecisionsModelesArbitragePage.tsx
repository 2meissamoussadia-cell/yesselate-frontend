/**
 * Page Modèles d'Arbitrage
 * Modèles d'arbitrage des décisions
 */
...
export const DecisionsModelesArbitragePage = memo(function DecisionsModelesArbitragePage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `modeles-arbitrage-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `modeles-arbitrage-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les modèles d\'arbitrage'}
          icon={Scale}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Modèles d'Arbitrage" description="Modèles d'arbitrage des décisions">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucun modèle d'arbitrage"
              description="Il n'y a actuellement aucun modèle d'arbitrage."
              icon={FileText}
              variant="info"
            />
          ) : (
