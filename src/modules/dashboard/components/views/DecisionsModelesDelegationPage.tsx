/**
 * Page Modèles de Délégation
 * Modèles de délégation des décisions
 */
...
export const DecisionsModelesDelegationPage = memo(function DecisionsModelesDelegationPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `modeles-delegation-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `modeles-delegation-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les modèles de délégation'}
          icon={UserPlus}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Modèles de Délégation" description="Modèles de délégation des décisions">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucun modèle de délégation"
              description="Il n'y a actuellement aucun modèle de délégation."
              icon={FileText}
              variant="info"
            />
          ) : (
