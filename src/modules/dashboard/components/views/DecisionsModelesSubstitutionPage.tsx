/**
 * Page Modèles de Substitution
 * Modèles de substitution des décisions
 */
...
export const DecisionsModelesSubstitutionPage = memo(function DecisionsModelesSubstitutionPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `modeles-substitution-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `modeles-substitution-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les modèles de substitution'}
          icon={RefreshCw}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Modèles de Substitution" description="Modèles de substitution des décisions">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucun modèle de substitution"
              description="Il n'y a actuellement aucun modèle de substitution."
              icon={FileText}
              variant="info"
            />
          ) : (
