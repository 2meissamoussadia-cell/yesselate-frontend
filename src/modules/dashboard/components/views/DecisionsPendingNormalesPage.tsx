/**
 * Page Décisions Normales
 * Décisions en attente de niveau normal
 */
...
export const DecisionsPendingNormalesPage = memo(function DecisionsPendingNormalesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-normales-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-normales-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les décisions normales'}
          icon={FileCheck}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Décisions Normales" description="Décisions en attente de niveau normal">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune décision normale"
              description="Il n'y a actuellement aucune décision normale en attente."
              icon={FileText}
              variant="info"
            />
          ) : (
