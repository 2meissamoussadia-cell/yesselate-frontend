/**
 * Page Décisions Planifiées
 * Décisions planifiées à traiter
 */
...
export const DecisionsPendingPlanifieesPage = memo(function DecisionsPendingPlanifieesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-planifiees-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-planifiees-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les décisions planifiées'}
          icon={Calendar}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Décisions Planifiées" description="Décisions planifiées à traiter">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune décision planifiée"
              description="Il n'y a actuellement aucune décision planifiée."
              icon={FileText}
              variant="info"
            />
          ) : (
