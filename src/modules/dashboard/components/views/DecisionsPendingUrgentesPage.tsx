/**
 * Page Décisions Urgentes
 * Décisions en attente nécessitant une action urgente
 */
...
export const DecisionsPendingUrgentesPage = memo(function DecisionsPendingUrgentesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-urgentes-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-urgentes-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les décisions urgentes'}
          icon={AlertCircle}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Décisions Urgentes" description="Décisions en attente nécessitant une action urgente">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              ...
              description="Il n'y a actuellement aucune décision urgente en attente."
              icon={FileText}
              variant="info"
            />
          ) : (
