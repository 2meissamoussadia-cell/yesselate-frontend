...
export const DecisionsTimelineChronologiquePage = memo(function DecisionsTimelineChronologiquePage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-timeline-chronologique-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-timeline-chronologique-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger la timeline chronologique'}
          icon={ListOrdered}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Timeline Chronologique" description="Chronologie des décisions">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune donnée"
              description="Il n'y a actuellement aucune donnée dans la timeline."
              icon={FileText}
              variant="info"
            />
          ) : (
