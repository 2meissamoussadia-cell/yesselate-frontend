/**
 * Page Timeline Par Auteur
 * Décisions par auteur dans la timeline
 */
...
export const DecisionsTimelineParAuteurPage = memo(function DecisionsTimelineParAuteurPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-timeline-par-auteur-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-timeline-par-auteur-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger la timeline par auteur'}
          icon={User}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Timeline Par Auteur" description="Décisions par auteur dans la timeline">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune donnée"
              description="Il n'y a actuellement aucune donnée par auteur."
              icon={FileText}
              variant="info"
            />
          ) : (
