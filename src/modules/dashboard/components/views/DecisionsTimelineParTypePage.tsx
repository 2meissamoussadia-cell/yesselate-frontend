/**
 * Page Timeline Par Type
 * Décisions par type dans la timeline
 */
...
export const DecisionsTimelineParTypePage = memo(function DecisionsTimelineParTypePage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `decisions-timeline-par-type-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `decisions-timeline-par-type-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger la timeline par type'}
          icon={Layers}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Timeline Par Type" description="Décisions par type dans la timeline">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucune donnée"
              description="Il n'y a actuellement aucune donnée par type."
              icon={FileText}
              variant="info"
            />
          ) : (
