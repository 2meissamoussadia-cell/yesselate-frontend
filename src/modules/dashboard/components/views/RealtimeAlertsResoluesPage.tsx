/**
 * Page Alertes Résolues (Realtime)
...
export const RealtimeAlertsResoluesPage = memo(function RealtimeAlertsResoluesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `realtime-alertes-resolues-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `realtime-alertes-resolues-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les alertes résolues'}
          icon={CheckCircle2}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Alertes Résolues" description="Historique des alertes résolues">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              ...
              description="Il n'y a actuellement aucune alerte résolue."
              icon={FileText}
              variant="info"
            />
          ) : (
