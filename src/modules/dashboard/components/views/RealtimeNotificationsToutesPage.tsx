/**
 * Page Toutes les Notifications (Realtime)
...
export const RealtimeNotificationsToutesPage = memo(function RealtimeNotificationsToutesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `realtime-notifications-toutes-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `realtime-notifications-toutes-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Toutes les Notifications" description="Toutes les notifications en temps réel">
