/**
 * Page Configuration Synchronisation (Realtime)
...
export const RealtimeSyncConfigurationPage = memo(function RealtimeSyncConfigurationPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `realtime-sync-configuration-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `realtime-sync-configuration-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Configuration Synchronisation" description="Configuration de la synchronisation">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              ...
              description="Il n'y a actuellement aucune donnée de configuration."
              icon={FileText}
              variant="info"
            />
          ) : (
