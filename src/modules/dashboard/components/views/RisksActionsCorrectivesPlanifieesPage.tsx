/**
 * Page Actions Correctives Planifiées
 * Liste des actions correctives planifiées
 */
...
export const RisksActionsCorrectivesPlanifieesPage = memo(function RisksActionsCorrectivesPlanifieesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `actions-correctives-planifiees-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `actions-correctives-planifiees-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les actions correctives planifiées'}
          icon={CalendarCheck}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Actions Correctives Planifiées" description="Actions correctives à réaliser">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              ...
              description="Il n'y a actuellement aucune action corrective planifiée."
              icon={FileText}
              variant="info"
            />
          ) : (
