/**
 * Page Blocages (Risques)
 * Liste des risques de type blocage
 */
...
export const RisksTypeBlocagesPage = memo(function RisksTypeBlocagesPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `risques-blocages-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `risques-blocages-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les blocages'}
          icon={AlertTriangle}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Blocages" description="Risques de type blocage à lever">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucun blocage"
              description="Il n'y a actuellement aucun risque de type blocage."
              icon={FileText}
              variant="info"
            />
          ) : (
