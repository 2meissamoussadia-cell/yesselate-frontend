/**
 * Page Contrats Expirés (Risques)
 * Liste des risques liés aux contrats expirés
 */
...
export const RisksTypeContratsExpiresPage = memo(function RisksTypeContratsExpiresPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `risques-contrats-expires-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `risques-contrats-expires-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les contrats expirés'}
          icon={FileSignature}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Contrats Expirés" description="Risques liés aux contrats expirés ou à renouveler">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              title="Aucun contrat expiré"
              description="Il n'y a actuellement aucun risque de contrat expiré."
              icon={FileText}
              variant="info"
            />
          ) : (
