/**
 * Page Paiements en Retard (Risques)
 * Liste des risques liés aux paiements en retard
 */
...
export const RisksTypePaiementsRetardPage = memo(function RisksTypePaiementsRetardPage() {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    exportToCSV(rowsCsv, headers, `risques-paiements-retard-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredRows]);

  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredRows, `risques-paiements-retard-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredRows]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les paiements en retard'}
          icon={DollarSign}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Paiements en Retard" description="Risques liés aux paiements en retard">
        ...
        <DashboardPanel>
          ...
          {filteredRows.length === 0 ? (
            <EmptyState
              ...
              description="Il n'y a actuellement aucun risque de paiement en retard."
              icon={FileText}
              variant="info"
            />
          ) : (
