 ...
 * Vue des tendances de performance par mois
 */
...
export const TrendsMensuellesPage = memo(function TrendsMensuellesPage() {
  ...
  const { data, isLoading, error } = useDashboardData<TrendsMensuellesData>();
  ...
  const filteredTrends = useMemo(() => {
    ...
    return data.trends.filter(trend => 
      ...
      trend.mois.toLowerCase().includes(query)
    );
  }, [data, searchQuery]);
  
  // ✅ Export CSV
  const handleExportCSV = useCallback(() => {
    const headers = ['Mois', 'Projets', 'Demandes', 'Budget consommé', 'Budget prévu', 'Retards', 'Score performance', 'Évolution %', 'Tendance'];
    ...
    exportToCSV(rows, headers, `trends-mensuelles-${new Date().toISOString().split('T')[0]}.csv`);
  }, [filteredTrends]);
  
  // ✅ Export JSON
  const handleExportJSON = useCallback(() => {
    exportToJSON(filteredTrends, `trends-mensuelles-${new Date().toISOString().split('T')[0]}.json`);
  }, [filteredTrends]);
    ...
    return (
      <DashboardPageLayout>
        <EmptyState
          ...
          description={error.message || 'Impossible de charger les tendances mensuelles'}
          icon={Calendar}
          variant="error"
        />
      </DashboardPageLayout>
  ...
  return (
    <DashboardPageLayout>
      <DashboardSection title="Tendances Mensuelles" description="Évolution de la performance par mois">
        ...
        <DashboardPanel>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-slate-200">Tendances par mois</h3>
          ...
          {filteredTrends.length === 0 ? (
            <EmptyState
              title={searchQuery ? "Aucune tendance trouvée" : "Aucune tendance mensuelle"}
              description={
                searchQuery
                  ? `Aucune tendance ne correspond à "${searchQuery}"`
                  : "Il n'y a actuellement aucune tendance mensuelle disponible."
              }
              icon={Calendar}
              variant="info"
            />
